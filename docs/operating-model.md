...
How a campus service desk runs the triage backend after deploy. Change metadata before you change Apex.
...
| Technician | Cases in the Campus IT app, queue membership | Interaction rows |
...
Read these from **Get Campus IT Operations Snapshot** or the Campus IT app, not from anecdotes.

| Signal | Healthy | Investigate |
| --- | --- | --- |
| Self-service deflection today | Catalog is matching common issues | Near 0% with high volume: the keywords are wrong, or callers are skipping the guide action |
| New escalations today | Tickets that need a human | Spike in one category: add or tighten a known-issue article |
| Oldest wait hours | Inside the SLA for that priority | Oldest High ticket past 4 hours: queue staffing, not routing |
...
Every action inserts a `Campus_IT_Interaction__c` row. The running user can create and read rows. Updates and deletes are blocked in a trigger so operations numbers cannot be edited after the fact.
...
## When to resolve in place vs open a ticket

The rule callers should follow, whatever the front end:
